...
import { Text, AlertDialog, AlertDialogBody, AlertDialogCloseButton, AlertDialogContent, AlertDialogFooter, AlertDialogHeader, AlertDialogOverlay, Avatar, Box, Button, Flex, Icon, Textarea, useColorModeValue, useDisclosure, Link } from '@chakra-ui/react'
import { Like, Comment } from '@/atoms/PostAtom'
...
const PostComment: FC<PostCommentProps> = ({
  ...
  const handleComment = async () => {
    ...
    await onComment(input, comment.postId, isReplying ? isChild ? comment.commentParentId : comment.commentId : 0);
    ...
    <div ref={commentRef} className='flex flex-col'>
      ...
      <div className='flex items-center'>
        ...
        <div className=' flex items-center gap-x-2'>
          ...
          <Link href={`/profile/${comment.author}`}>  <Text fontWeight={"semibold"}>{comment.author}</Text></Link>
          <p className='max-h-40 truncate text-xs text-zinc-500'>

          </p>
          <Text fontWeight={"light"} fontSize={12}>  {formatTimeToNow(new Date(comment.time))}</Text>
        </div>
      </div>
    ...
    text.startsWith("@") ? (
      ...
      <div className='flex '>
        ...
        <div className='flex items-center gap-2'>
          ...
          {user.userName == comment.author && <Button
            onClick={() => {
              handleDeleteComment()
            }}
            variant='outline'
