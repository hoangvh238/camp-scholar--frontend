...
const DocumentUpload: React.FC<DocumentProps> = ({ groupID }) => {
  ...
  const props: UploadProps = {
    ...
    onChange(info) {
      ...
      if (status === "done") {
        message.success(`${info.file.name} Đăng tải thành công.`);
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:8080/doc/document/" + groupID,
        formData,
        {
          headers: {
      ...
      >
        ...
        <Form.Item
          ...
          rules={[
            {
              required: true,
              message: "Mô tả tài liệu không được bỏ trống",
              type: "string",
            },
          ]}
        >
        ...
        <Form.Item
          ...
          rules={[
            {
              required: true,
              message: "Tài liệu đính kèm không được bỏ trống",
              type: "string",
            },
          ]}
        >
          <Dragger {...props} maxCount={1}>
            ...
            <p className="ant-upload-text">
              Kích hoặc thả file ở khu vực này để đăng tài liệu
            </p>
            <p className="ant-upload-hint">
              Chú ý : chỉ hỗ trợ đăng tài liệu định dạng PDF và WORD nên bạn hãy
              chú ý loại file khi đăng
            </p>
          </Dragger>
        </Form.Item>
