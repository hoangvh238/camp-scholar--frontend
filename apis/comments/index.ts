...
export const likeComment = (comment: number, user: number) => {
  ...
  return axiosClient.post(END_POINT.LIKE + "/" + comment + "?userid=" + user, {},{
...
export const disLikeComment = (comment: number, user: number) => {
  return axiosClient.post(END_POINT.DIS_LIKE + "/" + comment + "?userid=" + user, {},{
    headers: { Authorization: `Bearer ${getCookie("token")}` }, // Sử dụng "Bearer" trước token
  })
}

export const createComment = (content: string, userId:number, postId :number,parent:number) => {
  return axiosClient.post(END_POINT.COMMENT + "/" + postId + "/comments"+"?userid=" + userId, {
...
export const getAllComment = (postId:number ) => {
  return axiosClient.post(END_POINT.GET_ALL_COMMENT + "/" + postId , {
...
export const deleteComment = (commentId: number,userId:number) => {
  return axiosClient.post(END_POINT.DELETE + "/" + commentId+"?userid="+userId, {},{
    headers: { Authorization: `Bearer ${getCookie("token")}` }, // Sử dụng "Bearer" trước token
  });
};

// export const editComment = (content: string) => {
